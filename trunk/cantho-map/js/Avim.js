...
function AVIM()	{
	...
	this.db1 = [273,272];
	this.ds1 = ['d','D'];
	this.os1 = "o,O,o,O,ó,Ó,ò,Ò,?,?,?,?,õ,Õ,?,?,?,?,?,?,?,?,?,?".split(",");
	this.ob1 = "ô,Ô,ô,Ô,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?".split(",");
	this.mocs1 = "o,O,ô,Ô,u,U,ó,Ó,ò,Ò,?,?,?,?,õ,Õ,ú,Ú,ù,Ù,?,?,?,?,u,U,?,?,?,?,?,?,?,?,?,?".split(",");
	this.mocb1 = "o,O,o,O,u,U,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?".split(",");
	this.trangs1 = "a,A,â,Â,á,Á,à,À,?,?,?,?,ã,Ã,?,?,?,?,?,?,?,?,?,?".split(",");
	this.trangb1 = "a,A,a,A,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?".split(",");
	this.as1 = "a,A,a,A,á,Á,à,À,?,?,?,?,ã,Ã,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?".split(",");
	this.ab1 = "â,Â,â,Â,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,é,É,è,È,?,?,?,?,?,?".split(",");
	this.es1 = "e,E,é,É,è,È,?,?,?,?,?,?".split(",");
	this.eb1 = "ê,Ê,?,?,?,?,?,?,?,?,?,?".split(",");
	this.english = "ÐÂAOUÊÔ";
	this.lowen = "dâaouêô";
	this.arA = "á,à,?,ã,?,a,Á,À,?,Ã,?,A".split(',');
	this.mocrA = "ó,ò,?,õ,?,o,ú,ù,?,u,?,u,Ó,Ò,?,Õ,?,O,Ú,Ù,?,U,?,U".split(',');
	this.erA = "é,è,?,?,?,e,É,È,?,?,?,E".split(',');
	this.orA = "ó,ò,?,õ,?,o,Ó,Ò,?,Õ,?,O".split(',');
	this.aA = "?,?,?,?,?,â,?,?,?,?,?,Â".split(',');
	this.oA = "?,?,?,?,?,ô,?,?,?,?,?,Ô".split(',');
	this.mocA = "?,?,?,?,?,o,?,?,?,?,?,u,?,?,?,?,?,O,?,?,?,?,?,U".split(',');
	this.trangA = "?,?,?,?,?,a,?,?,?,?,?,A".split(',');
	this.eA = "?,?,?,?,?,ê,?,?,?,?,?,Ê".split(',');
	this.oA = "?,?,?,?,?,ô,?,?,?,?,?,Ô".split(',');
	...
	this.ckspell = function(w, k) {
		...
		var noNHE = "O,U,IE,Ô,O,U,IÊ,A,Â,UYE,UYÊ,UO,UO,UO,UO,UA,UA,OA,OE,OÊ".split(','),oMoc = "UU,UOU".split(',');
	...
	this.findC = function(w, k, sf) {
		...
		var str = "", res, cc = "", pc = "", tE = "", vowA = [], s = "ÂAÊÔOUêâaoôu", c = 0, dn = false, uw = this.up(w), tv, g;
		...
		var uk = this.up(k), uni_array = this.repSign(k), w2 = this.up(this.unV2(this.unV(w))), dont = "UA,UU".split(',');
			...
			if(!res) {
				for(g = 1; g <= w.length; g++) {
					...
					if(str.indexOf(uc) >= 0) {
						...
						if(!this.whit || (uw.indexOf("U") < 0) || (uw.indexOf("W") < 0)) {
							break;
						}
	...
	this.ie_replaceChar = function(w, pos, c) {
		...
		if(this.whit && (this.up(w.substr(w.length - pos - 1, 1)) == 'U') && (pos != 1) && (this.up(w.substr(w.length - pos - 2, 1)) != 'Q')) {
			this.whit = false;
			if((this.up(this.unV(this.fcc(c))) == "O") || (uc == "O")) {
	...
	this.up = function(w) {
		w = w.toUpperCase();
		if(this.isKHTML) {
			var str = "êôoâau????????????????????????", rep="ÊÔOÂAU???????????????????????", z, io;
	...
	this.AvimControlOn = function(e)
	{
	    AVIMObj.setMethod(0);
	    e.className = "AvimOn";
	    e.title = (nLanguage == 0)?"T?t b? gõ ti?ng Vi?t":"Turn off Vietnamese input method";
	    e.onclick = function(){AVIMObj.AvimControlOff(e)};
	}
	this.AvimControlOff = function(e)
	{
	    AVIMObj.setMethod(-1);
	    e.className = "AvimOff";
	    e.title = (nLanguage == 0)?"B?t b? gõ ti?ng Vi?t":"Turn on Vietnamese input method";
	    e.onclick = function(){AVIMObj.AvimControlOn(e)};
	}

	if (AVIMGlobalConfig.showControl)
	{
	    document.write('<span alt="" title="B?t b? gõ ti?ng Vi?t" class="AvimOff" id="avim" onclick="AVIMObj.AvimControlOn(this)"></span>');
	...
	if(AVIM.support && !AVIM.isKHTML) {
		...
		for(AVIM.g = 0; AVIM.g < AVIM.fID.length; AVIM.g++) {
			...
			if(AVIM.is_ie) {
				...
				if (doc && ((AVIM.up(doc.designMode) == "ON") || (doc.body != null && doc.body.contentEditable))) {
